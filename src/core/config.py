...
class Config:
    ...
    # --- Model cache (transient only) ---

    _transient_model_cache: Dict[str, Dict[str, List[str]]] = {}
    ...
    def set_cached_models(
        ...
        self._transient_model_cache[provider_type][capability] = list(models)

    # --- General settings ---

    def get_general_settings(self) -> GeneralSettings:
        g = self._get("general", {})
        # Note: migrating old default_user_prompt to both new prompts if it exists
        old_prompt = g.get("default_user_prompt", "")
