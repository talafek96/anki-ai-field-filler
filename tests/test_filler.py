"""Tests for response parsing, HTML conversion, and retry logic in Filler."""
...
class TestParseResponse:
    """Tests for Filler._parse_response."""
...
class TestRenderFlags:
    ...
    def test_image_flag_replaced(self, filler) -> None:
        ...
        with patch("src.core.filler.create_image_provider") as mock_factory:
            mock_prov = MagicMock()
            mock_prov.generate_image.return_value = b"\x89PNG_fake"
            mock_factory.return_value = mock_prov

            with patch("src.core.filler.Media.save_image") as mock_save:
    ...
    def test_audio_flag_replaced(self, filler) -> None:
        ...
        with patch("src.core.filler.create_tts_provider") as mock_factory:
            mock_prov = MagicMock()
    ...
    def test_mixed_flags(self, filler) -> None:
        ...
        with (
            patch("src.core.filler.create_image_provider") as mock_img_f,
            patch("src.core.filler.create_tts_provider") as mock_tts_f,
            patch("src.core.filler.Media.save_image") as mock_save_img,
            patch("src.core.filler.Media.save_audio") as mock_save_aud,
        ):
    ...
    def test_no_provider_configured_removes_flag(self, filler) -> None:
        ...
        html, errors = filler._render_flags("See {{IMAGE: something}} and hear {{AUDIO: text}}", "Field")
        assert errors == []
        assert "{{IMAGE" not in html
        assert "{{AUDIO" not in html
        assert "See " in html
...
class TestRenderRichContent:
    ...
    def test_flags_with_legacy_image_prompt(self, filler) -> None:
        ...
        with (
            patch("src.core.filler.create_image_provider") as mock_factory,
...
class TestGenerateAndParse:
    ...
    def test_succeeds_first_try(self, mock_create, filler) -> None:
        ...
        mock_provider.generate.return_value = json.dumps({"fields": {"A": {"content": "ok", "type": "text"}}})
