...
class TestClassifyOpenAI:
    ...
    @pytest.mark.parametrize("model_id", [
        "gpt-5.4", "gpt-5-mini", "gpt-4o", "gpt-4.1",
    ...
    @pytest.mark.parametrize("model_id", [
        "whisper-1", "text-embedding-3-large", "omni-moderation-latest",
...
class TestClassifyGoogle:
    ...
    def _model(self, name: str, desc: str = "", display: str = "",
               methods: list | None = None) -> tuple[dict, list]:
        m = {"name": f"models/{name}", "description": desc,
             "displayName": display}
    ...
    def test_imagen_model(self) -> None:
        m, methods = self._model(
            "imagen-4-preview", display="Imagen 4",
        )
        assert _classify_google_model(m, methods) == "image"
    ...
    def test_embedding_model_skipped(self) -> None:
        m, methods = self._model(
            "text-embedding-004", desc="Embedding model",
        )
